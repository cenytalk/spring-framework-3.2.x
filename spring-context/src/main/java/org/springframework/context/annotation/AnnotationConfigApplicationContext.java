...
public class AnnotationConfigApplicationContext extends GenericApplicationContext {
	...
	public AnnotationConfigApplicationContext() {
		//默认构造函数，初始化一个空容器，容器不包含任何Bean信息，需要在稍后通过调用其register()方法
		//注册配置类，并调用refresh()方法刷新容器，触发容器对注解Bean的载入、解析和注册过程
	...
	public AnnotationConfigApplicationContext(Class<?>... annotatedClasses) {
		//最常用的构造函数，通过将涉及到的配置类传递给该构造函数，以实现将相应配置类中的Bean注定注册到容器中
		this();
	...
	public AnnotationConfigApplicationContext(String... basePackages) {
		//该构造函数会自动扫描以给定的包及其子包下的所有类，并自动识别所有的Spring Bean，并将其注册到容器中
		this();
		scan(basePackages);
		refresh();
	}
