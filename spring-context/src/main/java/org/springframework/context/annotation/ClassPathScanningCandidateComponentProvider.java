...
public class ClassPathScanningCandidateComponentProvider implements EnvironmentCapable, ResourceLoaderAware {
	...
	//保存过滤规则要包含的注解，即Spring默认的@Component、@Repository、@Service、@Controller注解
	//的Bean,以及JavaEE6的@ManagedBean和JSR-330的@Named注解
	...
	protected void registerDefaultFilters() {//向容器注册过滤规则
	 ...
	 */
	public Set<BeanDefinition> findCandidateComponents(String basePackage) {//扫描给定类路径的包
		//创建存储扫描到的类的集合
		...
		try {
			...
			for (Resource resource : resources) {
				...
				if (resource.isReadable()) {
					try {
						//为指定资源获取元数据读取器，元信息读取器通过汇编读取资源元信息
